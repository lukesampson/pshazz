# based on Jeremy Skinner's HgTabExpansion from Posh-Hg
# https://github.com/JeremySkinner/posh-hg
param($fragment) # everything after ^hg\s*
...
function hgCommands($filter) {
    ...
    if($filter) {
         $hgCommands | Where-Object { $_.StartsWith($filter) } | ForEach-Object { $_.Trim() } | Sort-Object
    }
    else {
        $hgCommands | ForEach-Object { $_.Trim() } | Sort-Object
    }
}
...
function PopulateHgCommands() {
    ...
    if($global:PoshHgSettings.ShowPatches) {
        ...
        $hgCommands += (hg help mq) | ForEach-Object {
            if($_ -match '^ (\S+) (.*)') {
                    $matches[1]
             }
        }
...
function hgLocalBookmarks($filter) {
    hg bookmarks --quiet | ForEach-Object {
        if($_ -match "(\S+) .*") {
...
function hgflowStreams($filter) {
    ...
    if($filter) {
         $hgflowStreams | Where-Object { $_.StartsWith($filter) } | ForEach-Object { $_.Trim() } | Sort-Object
    }
    else {
        $hgflowStreams | ForEach-Object { $_.Trim() } | Sort-Object
    }
}
...
switch -regex ($fragment) {
    ...
    '^(push|pull|outgoing|incoming) (-\S* )*(\S*)$' {
        hgRemotes($matches[3])
    }
    ...
    '^flow (feature|release|hotfix|support) (\S*)$' {
