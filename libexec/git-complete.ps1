...
function gitAliases($filter) {
    git config --get-regexp "^alias.$filter" | ForEach-Object {
...
function gitStatus {
    git -c color.status=false status --short 2>$null | Select-String '^(.)(.) (.*?)(?: -> (.*))?$' | ForEach-Object {
        $_all, $groups = $_.matches.groups; # discard group 0 (complete match)
        ,($groups | ForEach-Object { $_.value })         # ',' forces array of arrays
    }
}

function gitIndexedFiles {
    gitStatus | Where-Object { $_[0] -and ($_[0] -ne '?') } | ForEach-Object { $_[2] }
}

function gitChangedFiles {
    gitStatus | Where-Object { $_[1] -ne ' ' } | ForEach-Object { $_[2] }
}

function gitStagedFiles {
    gitStatus | Where-Object { 'M','A','D','R','C' -contains $_[0] } | ForEach-Object { $_[2] }
}

function gitUnmergedFiles {
    gitStatus | Where-Object { $_[1] -eq 'U' } | ForEach-Object { $_[2] }
}

function gitDeletedFiles {
    gitStatus | Where-Object { $_[1] -eq 'D' } | ForEach-Object { $_[2] }
}
